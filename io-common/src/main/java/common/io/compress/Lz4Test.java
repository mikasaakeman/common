...
public class Lz4Test extends TestParent {
	...
	// Uncomment these tests if you want to see the performance of the less
	// efficient implementations.
	// Keep in mind that you will not get extra benefits from using those. The
	// only 2 options which affect
	// the output size are fast/high compressor and a compressor buffer size.
	// In terms of performance, you will be affected more by a change of a
	// slower compressor rather than
	...
	public int testFastNativeDouble64K() throws IOException {
		final LZ4Compressor compressor = LZ4Factory.nativeInstance()
				.fastCompressor();
		return baseBenchmark(new StreamFactory() {
			@Override
			public OutputStream getStream(OutputStream underlyingStream)
					throws IOException {
				return new LZ4BlockOutputStream(new LZ4BlockOutputStream(
						underlyingStream, BLOCK_64K, compressor), BLOCK_64K,
						compressor);
			}
		});
	}
	...
	public int testFastNativeDouble32M() throws IOException {
		...
		return baseBenchmark(new StreamFactory() {
			...
			public OutputStream getStream(OutputStream underlyingStream)
					throws IOException {
				return new LZ4BlockOutputStream(new LZ4BlockOutputStream(
						underlyingStream, MAX_BLOCK_SIZE, compressor),
						MAX_BLOCK_SIZE, compressor);
			}
		});
	}
	...
	public int testFastNativeTriple32M() throws IOException {
		...
		return baseBenchmark(new StreamFactory() {
			...
			public OutputStream getStream(OutputStream underlyingStream)
					throws IOException {
				return new LZ4BlockOutputStream(new LZ4BlockOutputStream(
						new LZ4BlockOutputStream(underlyingStream,
								MAX_BLOCK_SIZE, compressor), MAX_BLOCK_SIZE,
						compressor), MAX_BLOCK_SIZE, compressor);
			}
		});
	}

	private int lz4(final LZ4Compressor compressor, final int blockSize)
			throws IOException {
		return baseBenchmark(new StreamFactory() {
			@Override
			public OutputStream getStream(OutputStream underlyingStream)
					throws IOException {
				return new LZ4BlockOutputStream(underlyingStream, blockSize,
						compressor);
			}
		});
	}
